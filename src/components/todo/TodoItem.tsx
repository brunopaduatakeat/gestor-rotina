  ...
  return (
    <li
      className={`flex items-start gap-3 p-3 rounded-lg transition-colors ${
        todo.done ? 'opacity-50' : 'bg-slate-800/60 hover:bg-slate-800'
      }`}
    >
      <button
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${todo.done ? 'line-through text-slate-500' : 'text-slate-100'}`}>
          {todo.title}
        </p>
        {todo.dueDate && (
        ...
        {promoted && (
          <p className="text-xs text-blue-400 mt-0.5">↗ Promovido para Kanban</p>
        )}
      </div>
